...
- Requester Ticket Detail renders the reusable AttachmentSection component, which independently fetches its own attachment metadata rather than consuming the attachments array embedded in the Ticket Detail response. This keeps AttachmentSection decoupled and reusable at the cost of one extra API call per page view.
