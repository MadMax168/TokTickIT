...
- Cancel clears the in-progress selection and keeps the user on the selector. Lab 2 has no prior screen to return to, since this selector is the application's entry point; Cancel does not navigate elsewhere.
...
- The selected Requester is held in in-memory application state only (see `specification.md` §11) — it is not written to `localStorage`, `sessionStorage`, or a cookie, and does not survive a page reload.
...
- No-results explains that the current search/filter combination has no matches and provides Clear filters. The API returns the same response shape for both empty-list and no-results (see `api-spec.md` §6); the UI distinguishes them solely by whether search/filter controls are currently active.
...
- `artifacts/lab02/screenshots/create-ticket/`
- `artifacts/lab02/screenshots/my-tickets/`
- `artifacts/lab02/screenshots/ticket-detail/`
...
- Requester Selection: `Development Requester`, `Cancel`, `Continue`, and `Change Requester`.
