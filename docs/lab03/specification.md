...
| FR-10 | Allow IT Staff to claim, assign, and reassign a primary Ticket Owner from active IT Staff or Administrator users safely. | EP-14, EP-15 | UI-08 ownership | AC-16 | API-14, UI-08 |
...
| BR-13 | New tickets have no primary Ticket Owner. The owner target must be an active User with role IT_STAFF or ADMINISTRATOR. Only IT Staff may invoke claim/assign/reassign: claim selects the acting Staff user; assign/reassign may select either eligible role. Owner eligibility is distinct from permission to invoke a mutation; being assigned does not grant additional role privileges. Any active Staff member can manage any ticket regardless of current assignment. Ownership does not change status. Closed/Cancelled tickets cannot be reassigned. No unassign operation. |
...
| BR-21 | Deactivation retains all tickets, ownership, comments, and notes. Assigned inactive Staff or Administrators remain visible as inactive owners and can be reassigned by active Staff. Changing an assigned Staff or Administrator to REQUESTER is rejected while they own any ticket outside Closed/Cancelled; switching between the two eligible owner roles is permitted subject to Administrator safety rules. A historical terminal-ticket owner may retain attribution after becoming a Requester; that user is not eligible for new assignment. A Requester role change is rejected while any tickets reference them as requester. This avoids orphaning ownership access. |
...
| OPEN | IN_PROGRESS, WAITING_FOR_REQUESTER, CANCELLED | IT_STAFF | Active Ticket Owner (IT Staff or Administrator) required for IN_PROGRESS |
| IN_PROGRESS | WAITING_FOR_REQUESTER, RESOLVED, CANCELLED | IT_STAFF | Active Ticket Owner (IT Staff or Administrator) required for RESOLVED |
| WAITING_FOR_REQUESTER | IN_PROGRESS, RESOLVED, CANCELLED | IT_STAFF | Active Ticket Owner (IT Staff or Administrator) required for IN_PROGRESS or RESOLVED |
| RESOLVED | CLOSED, REOPENED | IT_STAFF | None beyond confirmation/reason rules |
| REOPENED | OPEN, IN_PROGRESS, CANCELLED | IT_STAFF | Active Ticket Owner (IT Staff or Administrator) required for IN_PROGRESS |
...
- Separate PublicComment and InternalNote models: integer ID, Ticket FK, authorId User FK, body, createdAt, nullable unique seedKey for development fixtures; index `(ticketId,createdAt,id)`. Relationships restrict user deletion and preserve attribution. User indexes support normalized email uniqueness and `(role,active,id)` lists. Session/author/assignee relationships are distinct from requester ownership.
- Migrate in a controlled maintenance window: back up DB and attachment store, preflight email normalization collisions and orphan references, abort rather than merge colliding accounts; copy all legacy identities and verify counts/FKs, then switch requester FK and retire the legacy table only after verification. Preserve ticket IDs/numbers/clientRequestIds/dates and all attachment rows/storage keys/files/removal metadata. Production migration creates no example comments or notes; the separate development seed below inserts explicit demo collaboration records. Failed migration rolls back database changes; do not modify stored files.
...
A minimum deterministic distribution follows; A–D are the existing active Requesters and S1–S3 are distinct active IT Staff, and AD1 is an active Administrator. Each row has a stable UUID clientRequestId used as its seed identity. Demo IT Priority starts equal to Requested Priority; tests separately exercise later changes. Ticket dates are fixed, valid UTC values before 2026-09-01T10:00:00Z so the collaboration examples below follow ticket creation.
...
| 12 | D | WAITING_FOR_REQUESTER | HIGH | AD1 | Awaiting Wi-Fi diagnostic details |
...
### Deterministic development collaboration seed

Insert these non-sensitive, plain-text examples only in development/test seeds. Authors reference the stable fixture users above; ticket references resolve through the demo ticket clientRequestId. Example timestamps must be after their ticket creation times. No real people, credentials, contact details or production incident data are included.

| Stable seed key | Channel | Demo ticket | Author | createdAt (UTC) | Exact body |
| --- | --- | --- | --- | --- | --- |
| lab03-public-01 | Public Comment | 03 | C (owning Requester) | 2026-09-01T10:00:00Z | The demo VPN disconnects after a few minutes. |
| lab03-public-02 | Public Comment | 03 | S1 | 2026-09-01T10:05:00Z | Please retry using the updated demo VPN profile. |
| lab03-public-03 | Public Comment | 12 | AD1 | 2026-09-01T10:10:00Z | Please confirm whether the demo Wi-Fi connection is stable. |
| lab03-note-01 | Internal Note | 03 | S1 | 2026-09-01T10:02:00Z | Demo triage: compare the VPN profile with the training configuration. |
| lab03-note-02 | Internal Note | 12 | AD1 | 2026-09-01T10:08:00Z | Demo triage: review the simulated Wi-Fi diagnostic results. |

Use a nullable unique seedKey on each collaboration model for these stable identities; normal API-created rows have seedKey=null. Seed keys are internal metadata, absent from API requests/responses. Insert missing demo records only; rerunning the seed must preserve existing IDs, bodies, author/ticket references and createdAt values, including deliberately modified fixture content, and never duplicate or overwrite user-created rows. Do not identify seed rows by mutable body text. Keep the normal API prohibition on editing comments/notes; preservation tests may modify fixtures directly in the isolated test database. Foreign keys must resolve to authorized authors: Public Comments use the owning Requester or Staff/Admin, and Internal Notes use Staff/Admin only.

Migration/seed tests must assert the exact initial examples, channel separation, stable row counts and IDs after two seed runs, preservation after fixture changes, and absence of demo collaboration rows in production migration. Authorized Requesters see only Public Comments on their own demo tickets; Internal Notes remain restricted to Staff/Admin.
...
| AC-16 | Given active Staff and a mutable ticket, when claiming/assigning/reassigning, then an active IT Staff or Administrator owner is saved without status change; stale claims, invalid targets and terminal tickets are rejected atomically. |
...
| AC-27 | Given a Lab 2 database and files, when migration and repeated seeding run, then IDs, ticket/attachment content, ownership and inactive flags are preserved; collision/orphan preflight fails safely; migrated login exposes only owned history and the selector endpoint no longer lists identities; IT Priority is backfilled from Requested Priority and development seeds meet the Staff counts, ticket distribution, and deterministic Public Comment/Internal Note examples in §7 without duplicates or overwritten collaboration content. |
